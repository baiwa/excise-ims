...
import { Cop0631Service } from 'projects/check-operation/cop06/cop06-3/cop06-3-1/cop06-3-1.service';
import { Cop0631Form } from 'projects/check-operation/cop06/cop06-3/cop06-3-1/cop06-3-1.modesl';
declare var $: any;
@Component({
  selector: 'cop06-3-1',
  templateUrl: './cop06-3-1.component.html',
  styleUrls: ['./cop06-3-1.component.css'],
  providers: [Cop0631Service]
})
export class Cop0631Component implements OnInit {
  ...
  breadcrumb: BreadCrumb[] = [
    ...
    { label: 'รายงานการตรวจปฏิบัติการการจ่ายสินค้าสำเร็จรูป', route: '#' },
  ]

  // === > params
  form: Cop0631Form = new Cop0631Form();
  exciseIdList: any;
  ...
  constructor(
    ...
    private cop0631Service: Cop0631Service,
  ) { }
  ...
  findExciseId = () => {
    this.cop0631Service.findExciseId().then((res) => {
  ...
  claer = () => {
    ...
    $("#Dtable").hide();
    this.cop0631Service.claer();
  }
  changeExciseId = (e) => {
    let exciseId = e.target.value;
    this.cop0631Service.findByExciseId(exciseId).then((res) => {
  ...
  onChangeUpload(file: any) {
    this.cop0631Service.onChangeUpload(file);
  }
  ...
  upload = () => {
    ...
    this.cop0631Service.upload(formBody);
    setTimeout(() => {
      this.loading = false;
    }, 500);
  }
  ...
  save = async () => {
    ...
    await this.cop0631Service.save().then( async(res)=>{
      if("C" == res){
