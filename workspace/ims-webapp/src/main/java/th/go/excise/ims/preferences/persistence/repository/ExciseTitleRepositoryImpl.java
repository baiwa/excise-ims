...
public class ExciseTitleRepositoryImpl implements ExciseTitleRepositoryCustom {
	...
	public void batchUpdate(List<Title> titleList) {
		...
		sql.append("     ET.TITLE_NAME = ?, ");
		sql.append("     ET.TITLE_SEQ = ?, ");
		sql.append("     ET.TITLE_TYPE = ?, ");
		sql.append("     ET.SUFFIX_NAME = ?, ");
		sql.append("     ET.SHORT_TITLE = ?, ");
		sql.append("     ET.SHORT_SUFFIX = ?, ");
		sql.append("     ET.IS_DELETED = 'N', ");
		sql.append("     ET.UPDATED_BY = ?, ");
		sql.append("     ET.UPDATED_DATE = ? ");
		sql.append(" WHEN NOT MATCHED THEN ");
		sql.append("   INSERT ("
				+ org.springframework.util.StringUtils.collectionToDelimitedString(insertColumnNames, ",") + ") ");
		sql.append("   VALUES (EXCISE_TITLE_SEQ.NEXTVAL"
				+ org.apache.commons.lang3.StringUtils.repeat(",?", insertColumnNames.size() - 1) + ") ");

		commonJdbcTemplate.batchUpdate(sql.toString(), titleList, BATCH_SIZE,
				new ParameterizedPreparedStatementSetter<Title>() {
