...
@Controller
@RequestMapping("api/oa/02/01/06/02")
public class Oa02010602Controller {
	
	private static final Logger logger = LoggerFactory.getLogger(Oa02010602Controller.class);
	
	@Autowired
	private Oa02010602Service oa02010602Service;
	...
	public ResponseData<OaLubricantsDtl> findDetailById(@PathVariable("id") String idStr) {
		...
		try {
			data = oa02010602Service.findDetailById(idStr);
	...
	public ResponseData<OaLubricantsDtl> updateById(@RequestBody Oa020106DtlVo request, @PathVariable("id") String idStr) {
		...
		OaLubricantsDtl data = new OaLubricantsDtl();
		try {
			data = oa02010602Service.updateById(request, idStr);
			responseData.setData(data);
