...
import th.go.excise.ims.oa.service.Oa01010602Service;
import th.go.excise.ims.oa.vo.Oa010106DtlVo;

@Controller
@RequestMapping("api/oa/01/01/06/02")
public class Oa01010602Controller {


	private static final Logger logger = LoggerFactory.getLogger(Oa02010602Controller.class);
	
	@Autowired
	private Oa01010602Service oa01010602Service;
	...
	public ResponseData<OaHydrocarbDtl> findDetailById(@PathVariable("id") String idStr) {
		...
		try {
			data = oa01010602Service.findDetailById(idStr);
	...
	public ResponseData<OaHydrocarbDtl> updateById(@RequestBody Oa010106DtlVo request, @PathVariable("id") String idStr) {
		...
		OaHydrocarbDtl data = new OaHydrocarbDtl();
		try {
			data = oa01010602Service.updateById(request, idStr);
			responseData.setData(data);
