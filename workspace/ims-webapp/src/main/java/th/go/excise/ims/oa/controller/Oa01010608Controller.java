...
import th.go.excise.ims.oa.service.Oa01010608Service;
import th.go.excise.ims.oa.vo.Oa010106DtlVo;

@Controller
@RequestMapping("api/oa/01/01/06/08")
public class Oa01010608Controller {
	private static final Logger logger = LoggerFactory.getLogger(Oa01010608Controller.class);

	@Autowired
	private Oa01010608Service oa01010608Service;
	...
	public ResponseData<OaHydrocarbDtl> findDetailById(@PathVariable("id") String idStr) {
		...
		try {
			data = oa01010608Service.findDetailById(idStr);
			responseData.setData(data);
			responseData.setStatus(RESPONSE_STATUS.SUCCESS);
		} catch (Exception e) {
			logger.error("Oa01010608Controller::findDetailById ", e);
	...
	public ResponseData<OaHydrocarbDtl> updateById(@RequestBody Oa010106DtlVo request,
		...
		OaHydrocarbDtl data = new OaHydrocarbDtl();
		try {
			data = oa01010608Service.updateById(request, idStr);
			responseData.setData(data);
			responseData.setMessage(ApplicationCache.getMessage(RESPONSE_MESSAGE.SAVE.SUCCESS_CODE).getMessageTh());
			responseData.setStatus(RESPONSE_STATUS.SUCCESS);
		} catch (Exception e) {
			logger.error("Oa01010608Controller::updateById ", e);
