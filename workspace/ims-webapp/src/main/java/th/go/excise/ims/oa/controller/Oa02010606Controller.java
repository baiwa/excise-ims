...
import th.go.excise.ims.oa.service.Oa02010606Service;
import th.go.excise.ims.oa.vo.Oa020106DtlVo;

@Controller
@RequestMapping("api/oa/02/01/06/06")
public class Oa02010606Controller {
	

	private static final Logger logger = LoggerFactory.getLogger(Oa02010606Controller.class);
	
	@Autowired
	private Oa02010606Service oa02010606Service;
	...
	public ResponseData<OaLubricantsDtl> findDetailById(@PathVariable("id") String idStr) {
		...
		try {
			data = oa02010606Service.findDetailById(idStr);
	...
	public ResponseData<OaLubricantsDtl> updateById(@RequestBody Oa020106DtlVo request, @PathVariable("id") String idStr) {
		...
		OaLubricantsDtl data = new OaLubricantsDtl();
		try {
			data = oa02010606Service.updateById(request, idStr);
			responseData.setData(data);
