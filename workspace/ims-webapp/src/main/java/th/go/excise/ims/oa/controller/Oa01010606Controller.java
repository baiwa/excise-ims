...
import th.go.excise.ims.oa.service.Oa01010606Service;
import th.go.excise.ims.oa.vo.Oa010106DtlVo;

@Controller
@RequestMapping("api/oa/01/01/06/06")
public class Oa01010606Controller {
	

	private static final Logger logger = LoggerFactory.getLogger(Oa02010606Controller.class);
	
	@Autowired
	private Oa01010606Service oa01010606Service;
	...
	public ResponseData<OaHydrocarbDtl> findDetailById(@PathVariable("id") String idStr) {
		...
		try {
			data = oa01010606Service.findDetailById(idStr);
	...
	@PutMapping("/save/{id}")
	@ResponseBody
	public ResponseData<OaHydrocarbDtl> updateById(@RequestBody Oa010106DtlVo request, @PathVariable("id") String idStr) {
		ResponseData<OaHydrocarbDtl> responseData = new ResponseData<OaHydrocarbDtl>();
		OaHydrocarbDtl data = new OaHydrocarbDtl();
		try {
			data = oa01010606Service.updateById(request, idStr);
			responseData.setData(data);
