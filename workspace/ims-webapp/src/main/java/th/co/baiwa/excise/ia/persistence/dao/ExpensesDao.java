...
public class ExpensesDao {
	...
	private RowMapper<LabelValueBean> yearRowmapper = new RowMapper<LabelValueBean>() {
		...
		public LabelValueBean mapRow(ResultSet rs, int arg1) throws SQLException {
			Date date = DateConstant.convertStrToDate(rs.getString("YEAR"), ExciseConstants.FORMAT_DATE.YYYYMMDD,
					ExciseConstants.LOCALE.EN);

			String strDate = DateConstant.convertDateToStr(date, ExciseConstants.FORMAT_DATE.YYYYMMDD,
					ExciseConstants.LOCALE.EN);
			String yyyyEn = DateConstant.convertDateToStr(date, ExciseConstants.FORMAT_DATE.YYYY,
					ExciseConstants.LOCALE.EN);
			String yyyyTh = DateConstant.convertDateToStr(date, ExciseConstants.FORMAT_DATE.YYYY,
			...
			if (intDate >= intYyyy) {
				result = DateConstant.convertDateToStr(DateUtils.addYears(date, 1), ExciseConstants.FORMAT_DATE.YYYY,
						ExciseConstants.LOCALE.TH);
