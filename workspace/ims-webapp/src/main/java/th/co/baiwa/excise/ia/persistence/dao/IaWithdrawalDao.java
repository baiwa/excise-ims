...
public class IaWithdrawalDao {
	...
	private final String SQL = "SELECT ps.PAYMENT_DATE PAYMENT_DATE , " + "        ps.WITHDRAWAL_ID WITHDRAWAL_ID, "
			+ "        ps.REF_PAYMENT REF_PAYMENT, " + "        ps.PAYEE PAYEE, " + "        ps.BANK_NAME BANK_NAME, "
			+ "        LS.ITEM_DESC ITEM_DESC," + "        ps.OFFICE_CODE OFFICE_CODE, "
			+ "        LS.BUDGET_TYPE BUDGET_TYPE,  " + "        ps.AMOUNT  AMOUNT" + "        FROM  "
			+ "   		(SELECT TO_CHAR(ps.PAYMENT_DATE,'YYYYMMDD')PAYMENT_DATE , "
			+ "            ps.WITHDRAWAL_ID WITHDRAWAL_ID, " + "            ps.REF_PAYMENT REF_PAYMENT, "
			+ "            ps.OFFICE_CODE," + "            ps.BANK_NAME BANK_NAME, " + "            ps.PAYEE PAYEE, "
			+ "            ps.AMOUNT AMOUNT" + "   FROM IA_WITHDRAWAL_PERSONS ps WHERE PAYMENT_METHOD = ? ) PS  "
			+ "   LEFT JOIN  " + "   (SELECT WITHDRAWAL_ID,ITEM_DESC,BUDGET_TYPE " + "        FROM IA_WITHDRAWAL_LIST "
			+ "        WHERE WITHDRAWAL_ID in (SELECT WITHDRAWAL_ID FROM IA_WITHDRAWAL_PERSONS WHERE PAYMENT_METHOD = ? GROUP BY WITHDRAWAL_ID  "
			+ "    )) LS " + "    ON  LS.WITHDRAWAL_ID=ps.WITHDRAWAL_ID WHERE 1=1";
	...
	public List<Int065Vo> findAllKtb(Int065FormVo formVo) {
		...
		System.out.println(ToStringBuilder.reflectionToString(formVo, ToStringStyle.JSON_STYLE));
	...
	private RowMapper<Int065Vo> stamRowmapper = new RowMapper<Int065Vo>() {
		...
		public Int065Vo mapRow(ResultSet rs, int arg1) throws SQLException {
			...
			Date date = DateConstant.convertStrToDate(rs.getString("PAYMENT_DATE"),
					ExciseConstants.FORMAT_DATE.YYYYMMDD, ExciseConstants.LOCALE.EN);
			String dateStr = DateConstant.convertDateToStr(date, ExciseConstants.FORMAT_DATE.DDMMYYYY,
					ExciseConstants.LOCALE.TH);
