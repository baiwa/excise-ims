...
public class Int0614Dao {
	...
	private final String SQL = " select * from("
			...
			+ " and BELONG = ? " + "   )T1 Group by T1.NAME, T1.FLAG" + " ) where 1 = 1 ";

	public List<Int0614FormSearchVo> findInRentHouse(Int0614FormSearchVo formVo) {

		List<Object> valueList = new ArrayList<Object>();
		StringBuilder sql = new StringBuilder(SQL);

		valueList.add(formVo.getDateStart());
		valueList.add(formVo.getDateEnd());
		valueList.add(formVo.getSector());
		...
		if(StringUtils.isNotBlank(formVo.getWithdrawAmount())){
			sql.append(" AND count >= ? ");
			valueList.add(formVo.getWithdrawAmount());
		}
