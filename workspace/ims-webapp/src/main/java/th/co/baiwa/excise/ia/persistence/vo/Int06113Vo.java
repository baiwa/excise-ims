package th.co.baiwa.excise.ia.persistence.vo;

public class Int06113Vo {
    private String id;
    private String accountId;
    private String accountName;
    private String serviceReceive;
    private String serviceWithdraw;
    private String serviceBalance;
    private String suppressReceive;
    private String suppressWithdraw;
    private String suppressBalance;
    private String budgetReceive;
    private String budgetWithdraw;
    private String budgetBalance;
    private String sumReceive;
    private String sumWithdraw;
    private String sumBalance;
    private String moneyBudget;
    private String moneyOut;
    private String averageCost;
    private String averageGive;
    private String averageFrom;
    private String averageComeCost;
    private String createdBy;
    private String createdDate;
    private String note;
    private String experimentalBudget;
    private String ledger;
    private String differenceExperimentalBudget;
    private String differenceledger;

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getAccountId() {
        return accountId;
    }

    public void setAccountId(String accountId) {
        this.accountId = accountId;
    }

    public String getAccountName() {
        return accountName;
    }

    public void setAccountName(String accountName) {
        this.accountName = accountName;
    }

    public String getServiceReceive() {
        return serviceReceive;
    }

    public void setServiceReceive(String serviceReceive) {
        this.serviceReceive = serviceReceive;
    }

    public String getServiceWithdraw() {
        return serviceWithdraw;
    }

    public void setServiceWithdraw(String serviceWithdraw) {
        this.serviceWithdraw = serviceWithdraw;
    }

    public String getServiceBalance() {
        return serviceBalance;
    }

    public void setServiceBalance(String serviceBalance) {
        this.serviceBalance = serviceBalance;
    }

    public String getSuppressReceive() {
        return suppressReceive;
    }

    public void setSuppressReceive(String suppressReceive) {
        this.suppressReceive = suppressReceive;
    }

    public String getSuppressWithdraw() {
        return suppressWithdraw;
    }

    public void setSuppressWithdraw(String suppressWithdraw) {
        this.suppressWithdraw = suppressWithdraw;
    }

    public String getSuppressBalance() {
        return suppressBalance;
    }

    public void setSuppressBalance(String suppressBalance) {
        this.suppressBalance = suppressBalance;
    }

    public String getBudgetReceive() {
        return budgetReceive;
    }

    public void setBudgetReceive(String budgetReceive) {
        this.budgetReceive = budgetReceive;
    }

    public String getBudgetWithdraw() {
        return budgetWithdraw;
    }

    public void setBudgetWithdraw(String budgetWithdraw) {
        this.budgetWithdraw = budgetWithdraw;
    }

    public String getBudgetBalance() {
        return budgetBalance;
    }

    public void setBudgetBalance(String budgetBalance) {
        this.budgetBalance = budgetBalance;
    }

    public String getSumReceive() {
        return sumReceive;
    }

    public void setSumReceive(String sumReceive) {
        this.sumReceive = sumReceive;
    }

    public String getSumWithdraw() {
        return sumWithdraw;
    }

    public void setSumWithdraw(String sumWithdraw) {
        this.sumWithdraw = sumWithdraw;
    }

    public String getSumBalance() {
        return sumBalance;
    }

    public void setSumBalance(String sumBalance) {
        this.sumBalance = sumBalance;
    }

    public String getMoneyBudget() {
        return moneyBudget;
    }

    public void setMoneyBudget(String moneyBudget) {
        this.moneyBudget = moneyBudget;
    }

    public String getMoneyOut() {
        return moneyOut;
    }

    public void setMoneyOut(String moneyOut) {
        this.moneyOut = moneyOut;
    }

    public String getAverageCost() {
        return averageCost;
    }

    public void setAverageCost(String averageCost) {
        this.averageCost = averageCost;
    }

    public String getAverageGive() {
        return averageGive;
    }

    public void setAverageGive(String averageGive) {
        this.averageGive = averageGive;
    }

    public String getAverageFrom() {
        return averageFrom;
    }

    public void setAverageFrom(String averageFrom) {
        this.averageFrom = averageFrom;
    }

    public String getAverageComeCost() {
        return averageComeCost;
    }

    public void setAverageComeCost(String averageComeCost) {
        this.averageComeCost = averageComeCost;
    }

    public String getCreatedBy() {
        return createdBy;
    }

    public void setCreatedBy(String createdBy) {
        this.createdBy = createdBy;
    }

    public String getCreatedDate() {
        return createdDate;
    }

    public void setCreatedDate(String createdDate) {
        this.createdDate = createdDate;
    }

    public String getNote() {
        return note;
    }

    public void setNote(String note) {
        this.note = note;
    }

    public String getExperimentalBudget() {
        return experimentalBudget;
    }

    public void setExperimentalBudget(String experimentalBudget) {
        this.experimentalBudget = experimentalBudget;
    }

    public String getLedger() {
        return ledger;
    }

    public void setLedger(String ledger) {
        this.ledger = ledger;
    }

    public String getDifferenceExperimentalBudget() {
        return differenceExperimentalBudget;
    }

    public void setDifferenceExperimentalBudget(String differenceExperimentalBudget) {
        this.differenceExperimentalBudget = differenceExperimentalBudget;
    }

    public String getDifferenceledger() {
        return differenceledger;
    }

    public void setDifferenceledger(String differenceledger) {
        this.differenceledger = differenceledger;
    }
}
