...
//	}
	...
	public void add(Int06101FormVo formVo) throws Exception {
		String officeCode = UserLoginUtils.getCurrentUserBean().getOfficeId();
		String officeDesc = ApplicationCache.getListOfValueByValueType("SECTOR_LIST", officeCode).get(0)
				.getTypeDescription();
	...
	public void update(Int06101FormVoRequest vo, Long id) throws Exception {
		String officeCode = UserLoginUtils.getCurrentUserBean().getOfficeId();
		String officeDesc = ApplicationCache.getListOfValueByValueType("SECTOR_LIST", officeCode).get(0)
				.getTypeDescription();
		Int06101FormVo formVo = vo.getData();
		...
		if (vo.getDelete().size() > 0) {
			for (Long personId : vo.getDelete()) {
				personsRep.delete(personId);
			}
		}
		if (formVo.getPersons().size() > 0) {
			personsRep.save(formVo.getPersons());
