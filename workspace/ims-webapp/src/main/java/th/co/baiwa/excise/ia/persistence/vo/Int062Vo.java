package th.co.baiwa.excise.ia.persistence.vo;

import java.util.List;

import th.co.baiwa.excise.ia.persistence.entity.CwpScwdDtl;

public class Int062Vo {
	private CwpScwdDtl cwpScwdDtl;
	private Long fileUploadID;
	private List<CwpScwdDtl> cwpScwdDtlList;

	public CwpScwdDtl getCwpScwdDtl() {
		return cwpScwdDtl;
	}

	public void setCwpScwdDtl(CwpScwdDtl cwpScwdDtl) {
		this.cwpScwdDtl = cwpScwdDtl;
	}

	public Long getFileUploadID() {
		return fileUploadID;
	}

	public void setFileUploadID(Long fileUploadID) {
		this.fileUploadID = fileUploadID;
	}

	public List<CwpScwdDtl> getCwpScwdDtlList() {
		return cwpScwdDtlList;
	}

	public void setCwpScwdDtlList(List<CwpScwdDtl> cwpScwdDtlList) {
		this.cwpScwdDtlList = cwpScwdDtlList;
	}

}
